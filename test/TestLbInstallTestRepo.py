...
import unittest
import shutil
from LbInstall import InstallProjectClient, LbInstallClient
...
#        client = LbInstallClient(TESTCONFIG, args)
#        rc =  client.main()
...
#            self.assertTrue(os.path.exists(os.path.join(self.siteroot, path)))
    ...
    def testConfigInstallOldReleaseAndUpgrade(self):
        ...
        client = LbInstallClient(TESTCONFIG, ["--root=%s" % self.siteroot, "--noautoupdate", "-d",  "rpm", "-qa"])
        rc =  client.main()
        self.assertEquals(rc, 0)
        # package should be installed, checking that the file is correct
        self._checkTestRPM1Release("1")
        ...
        client = LbInstallClient(TESTCONFIG, ["--root=%s" % self.siteroot, "-d",  "rpm", "-qa"])
        rc =  client.main()
        self.assertEquals(rc, 0)
        # package should be installed, checking that the file is correct
